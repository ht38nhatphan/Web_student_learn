...
export default function LessonEngine({ challenge, userId, onComplete, onPenalty, onBack }: Props) {
  ...
  const [hasQuestionsInDB, setHasQuestionsInDB] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    let totalInDB = 0;
    if (data.multiplechoice[challenge.id]) totalInDB += data.multiplechoice[challenge.id].length;
    if (data.fillblank[challenge.id]) totalInDB += data.fillblank[challenge.id].length;
    if (data.truefalse[challenge.id]) totalInDB += data.truefalse[challenge.id].length;
    if (data.typing[challenge.id]) totalInDB += data.typing[challenge.id].length;
    if (data.reorder[challenge.id]) totalInDB += data.reorder[challenge.id].length;
    if (data.matchword[challenge.id]) totalInDB += data.matchword[challenge.id].length;
    
    setHasQuestionsInDB(totalInDB > 0);
  ...
  // ── Empty state ─────────────────────────────────────────────────────────────
  if (items.length === 0) {
    if (hasQuestionsInDB === null) return null; // đang load
    if (hasQuestionsInDB) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center bg-white p-8 rounded-3xl mt-8 shadow-sm">
          <div className="text-6xl mb-4">🏆</div>
          <h2 className="text-2xl font-black text-green-500 mb-2 text-center">Hoàn thành xuất sắc!</h2>
          <p className="text-gray-500 font-medium text-center mb-6">Con đã trả lời đúng tất cả các câu hỏi trong thử thách này rồi.</p>
          <button onClick={onBack} className="px-6 py-3 bg-purple-600 font-black text-white rounded-2xl hover:bg-purple-700 shadow-lg hover:-translate-y-1 transition-transform">Chọn bài khác</button>
        </div>
      );
    }
    return (
